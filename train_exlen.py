...
def run_experiment(cfg: TrainConfig, max_steps: int, log_interval: int, eval_interval : int, save_interval: int, device: torch.device, results_filename: str, use_streaming: bool):
    ...
    print(f"Using Position Embedding: {'RoPE' if cfg.model.rope else 'FoPE or Grid'}")
    if cfg.model.rope:
    ...
    results = {}
    save_dir = f"/home/qijunrong/03-proj/PE/checkpoints/{cfg.run_name}"
    ...
    if max_steps + 1 % eval_interval != 0:
...
def main():
    ...
    parser.add_argument("--use_diag_pe", action="store_true", help="Enable diagonal position encoding.")
    ...
    parser.add_argument("--grid_sigma",type=float,default=None,help = "Place field size for GridEmbedding")
    ...
    if args.use_diag_pe:
        ...
        elif args.position_embedding == 'grid':
            cfg.model.fope = False
            cfg.model.rope = False
            cfg.model.grid = True
            cfg.model.grid_sigma = args.grid_sigma
            if args.decay_func is not None:
                cfg.model.decay_func = args.decay_func
