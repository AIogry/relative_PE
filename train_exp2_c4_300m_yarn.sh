...
#SBATCH --job-name=exp2-c4-yarn
#SBATCH --output=/data/qijunrong/03-proj/PE/logs/exp2_c4_300M/tmp_yarn_%j.out
#SBATCH --error=/data/qijunrong/03-proj/PE/logs/exp2_c4_300M/tmp_yarn_%j.err
...
JOB_ID=${SLURM_JOB_ID}
FINAL_OUT="${LOG_DIR}/exp2_yarn_${JOB_ID}.out"
FINAL_ERR="${LOG_DIR}/exp2_yarn_${JOB_ID}.err"

exec > >(tee -a ${FINAL_OUT}) 2> >(tee -a ${FINAL_ERR} >&2)

function cleanup {
    rm -f /data/qijunrong/03-proj/PE/logs/exp2_c4_300M/tmp_yarn_${JOB_ID}.out
    rm -f /data/qijunrong/03-proj/PE/logs/exp2_c4_300M/tmp_yarn_${JOB_ID}.err
}
trap cleanup EXIT
        ...
        for M_SIZE in "${MODELS[@]}"; do
            for SEQ_LEN in "${LENGTHS[@]}"; do
                ...
                $PYTHON_BIN $SCRIPT \
                    ...
                    --wandb_dir $WANDB_DIR --wandb_mode "offline" \
                    $extra_args \
...
# 直接调用 YaRN 实验
run_baseline_experiment "yarn" "--yarn"

echo -e "\n>>> YaRN 300M Experiments Completed."