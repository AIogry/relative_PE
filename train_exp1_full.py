...
# --- Dataset 1: 标准关联回忆 (Standard / One-to-One) ---
...
# --- Dataset 2: 块复制任务 (Block / One-to-Many) ---
...
def main():
    ...
    # 任务类型
    # standard = exp1-1 (One-to-One)
    # block = exp1-2 (One-to-Many)
    ...
    if args.model_size == "20M":
        ...
        # 60M 配置 (Wide)
        d_model = 512
        mlp_ratio = 4 # hidden = 2048 (保持参数量约 60M)
    ...
    cfg = ModelConfig(
        ...
        rope_scaling_threshold=args.rope_scaling_threshold, # 传入阈值
    ...
    for step in range(args.steps):
        ...
        with autocast(device_type='cuda', dtype=torch.bfloat16):
            ...
            if loss_mask is not None:
                ...
                # Standard 任务逻辑
                # 取出最后两个位置 (Query Key -> Query Value)
                # input: [k1, v1, ..., k_q, v_q]
                # target index for v_q is the last position
                # 注意：AssociativeRecallDataset 构造是 len=seq_len
                # 最后一个 token 是 v_q, 倒数第二个是 k_q
                # 我们想预测的是 v_q
                
                # Dataset 逻辑: sequence.append(keys[query_idx]); sequence.append(vals[query_idx])
                # 所以 target 的最后一个值就是我们要预测的 value
                
                # 简单起见，计算所有有效位置的 Loss 也是可以的，
                # 但为了与原始 exp1 保持一致，这里只关注最后的预测
                ...
                # 为了更通用，取最后一个非 padding 的位置
                # 这里假设 batch 内长度对齐，直接取最后
                # input_ids: [..., K, V, padding]
                # 我们这里 seq_len 比较短，padding 较少，直接用最后一个有效位
                
                # 简化逻辑：计算序列最后一次预测的 loss
                # 实际上对于 synthetic task，只要最后预测对了就行
                # 这里我们稍微放宽，计算最后 token 的 loss
                
                # 重新复用原代码逻辑（假设 seq_len 足够容纳 num_pairs）
