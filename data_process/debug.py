...
def run_experiment(cfg: TrainConfig, max_steps: int, save_interval: int, device: torch.device, results_filename: str, use_streaming: bool):
    ...
    print("="*80)
    print(f"RoPE: {cfg.model.rope}, ScaledRoPE: {cfg.model.use_scaled_rope}")
    print(f"sin_lambda: {cfg.model.sin_lambda}, cos_sigma: {cfg.model.cos_sigma}, scaled_rope_sigma: {cfg.model.scaled_rope_sigma}, decay_func: {cfg.model.decay_func}")
    print("="*80)
    ...
    hf_train_datasets = [load_from_disk("/data/qijunrong/03-proj/PE/c4_train_shards").select(range(1000))]
    eval_dataset = load_from_disk("/data/qijunrong/03-proj/PE/c4_val_shards").select(range(1000))
...
def main():
    ...
    parser.add_argument("--run_name", type=str, default="olmo-60m-debug", help="Custom name for the run. Overrides the name in the config.")
