"""
Preprocess test data with 50k samples.
放在03-proj下运行
"""
import os
import pickle
...
def preprocess_test_cache(
    test_path, cache_dir="/data/qijunrong/03-proj/PE/preprocessed_data", 
    max_seq_length=512, test_subset_size=50000
):
    """
    Create cache file for test data with 50k samples.
    """
    ...
    # Create test cache with 50k samples
    test_cache_path = tokenize_and_chunk_subset(
        test_dataset, "test", test_subset_size, len(test_dataset), 
        desc_suffix=f" (max: {test_subset_size})"
    )
...
if __name__ == "__main__":
    test_path = "/data/qijunrong/03-proj/PE/c4_30M_test"
    
    # Create cache for test set
    test_cache_path = preprocess_test_cache(
        test_path=test_path,
        cache_dir="/data/qijunrong/03-proj/PE/preprocessed_data",
        max_seq_length=512,
        test_subset_size=50000
    )
    
    print("Test caching completed!")
