...
class ModelConfig(BaseConfig):
    ...
    """
    diag position encoding
    """

    """
    Use place cells (rescale the fope)
    """
    place_cell_sigma: float = 0.1
    """
    The size of the place field
    """

    rope_fourier_init_norm_gain: float =  1.
    """
    Parameter for FoPE
    """
    ...
    grid_sigma: float = 30.0
    """
    For grid embedding sigma
    """

    yarn_enabled: bool = False
    """
    Enable YaRN (Yet another RoPE extensioN) for context window extension.
    """
    yarn_max_position_embeddings: int = 512
    """
    Original training context length (L_base).
    """
    yarn_target_max_position_embeddings: Optional[int] = None
    """
    Target context length for fine-tuning (L_target). If None, inferred from max_sequence_length.
    """
    yarn_beta_fast: float = 32.0
    """
    beta in YaRN ramp function (high-frequency cutoff).
    """
    yarn_beta_slow: float = 1.0
    """
    alpha in YaRN ramp function (low-frequency start).
    """

    yarn_dynamic_scaling: bool = False
    """
    If True, use dynamic scaling at inference time (no fine-tuning needed).
    """


    use_scaled_rope2: bool = False
    """
    Use Scaled Rotary Positional Embeddings (Scaled-RoPE).
    This is a variant of RoPE that applies a variance-preserving, frequency-based
    scaling to queries and keys. Requires `rope` to be `True` and `fope` to be `False`.
    """
    scaled_rope_sigma: float = 1.0
    """
    The sigma value to use for Scaled-RoPE. Only used when `use_scaled_rope` is `True`.
    """
...
# In olmo/config.py, inside the ModelConfig class
    ...
    """
    A list of sigma values for Scaled-RoPE, with one value per attention head.
    If provided, this list takes precedence. Only used when `use_scaled_rope` is `True`.
    """
    decay_func: Optional[str] = None
    """
    A list of ScaledRotaryEmbedding class sigma which is for weight decay in the omega(inv_freq)
    """
    ...
    """
    cos_sigma: float = 20.0
    """
    for scaling cos term into gaussian
    """
