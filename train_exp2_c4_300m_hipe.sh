...
#SBATCH --output=/data/qijunrong/03-proj/PE/logs/exp2_c4_300M/tmp_hipe_%j.out
#SBATCH --error=/data/qijunrong/03-proj/PE/logs/exp2_c4_300M/tmp_hipe_%j.err
...
# 指向我们刚刚为你生成的 C4 专用 Python 脚本
SCRIPT="train_exp2_c4full.py" 

# === 路径配置 ===
ROOT_DIR="/data/qijunrong/03-proj/PE"
# 将日志、权重和 WandB 目录加上 c4 前缀，与 wikitext 彻底隔离
...
function cleanup {
    rm -f /data/qijunrong/03-proj/PE/logs/exp2_c4_300M/tmp_hipe_${JOB_ID}.out
    rm -f /data/qijunrong/03-proj/PE/logs/exp2_c4_300M/tmp_hipe_${JOB_ID}.err
}
trap cleanup EXIT
...
run_experiment() {
    ...
    if [ $? -ne 0 ]; then
        echo ">>> [ERROR] C4 HIPE实验失败！Model: $M_SIZE, Len: $SEQ_LEN, Sigma: $SIGMA, SEED: $SEED"
    fi
}
