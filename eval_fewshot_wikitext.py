"""
Few-Shot外推评估脚本 - WikiText版本
使用WikiText-103进行few-shot和测试（与C4不同领域，避免数据污染）
"""
import torch
...
def main():
    parser = argparse.ArgumentParser(description="Few-Shot Extrapolation Evaluation (WikiText)")
    ...
    # WikiText数据路径
    parser.add_argument("--wikitext_path", type=str, 
                        default="/data/qijunrong/03-proj/PE/wikitext/raw",
                        help="WikiText-103数据集路径")
    parser.add_argument("--local_tokenizer_path", type=str, required=True)
    
    # Few-shot配置
    parser.add_argument("--few_shot_k", type=int, default=5000,
                        help="Few-shot样本数量（从WikiText train采样）")
    parser.add_argument("--few_shot_steps", type=int, default=300)
    ...
    parser.add_argument("--output_file", type=str, default="wikitext_extrap_results.json")
    ...
    print("="*60)
    print(f"Few-Shot Extrapolation Evaluation (WikiText-103)")
    print(f"Data: WikiText (different domain from C4 pretraining)")
    ...
    # 加载WikiText数据
    print(f"Loading WikiText from: {args.wikitext_path}")
    dataset_dict = load_from_disk(args.wikitext_path)
    ...
    # 过滤空样本
    train_full = train_full.filter(lambda x: len(x['text']) > 0, num_proc=4)
    val_full = val_full.filter(lambda x: len(x['text']) > 0, num_proc=4)
    
    print(f"  Train (non-empty): {len(train_full)} samples")
    print(f"  Val (non-empty): {len(val_full)} samples")
    
    # 从WikiText train采样few-shot数据（C4预训练模型未见过）
    ...
    # 测试数据用WikiText validation
    test_ds = val_full
    
    print(f"  Few-shot: {len(few_shot_ds)} samples (from WikiText train)")
    print(f"  Test: {len(test_ds)} samples (from WikiText val)")
    ...
    print(f"   Domain: WikiText (different from C4 pretraining)")
    print(f"{'='*60}")
