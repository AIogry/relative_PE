...
def main():
    ...
    while step < total_steps:
        ...
        if step % args.eval_interval == 0:
            model.eval()
            total_val_loss = 0.0
            total_val_tokens = 0  # 记录总 Token 数
            
            with torch.no_grad():
                for vx, vy in val_loader:  # 遍历整个验证集
                    vx, vy = vx.to(device), vy.to(device)
                    with autocast(device_type='cuda', dtype=torch.bfloat16):
                        outputs = model(input_ids=vx)
                        # 注意这里：改为 reduction='sum'，求该批次所有 token 的 loss 总和
                    ...
                    total_val_tokens += vy.numel() # 累加当前批次的 token 总数量
            
            if total_val_tokens > 0:
                # 最终 PPL = exp(总 Loss / 总 Token 数)
