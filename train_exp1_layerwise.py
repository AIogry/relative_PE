...
class AssociativeRecallDataset(IterableDataset):
    def __init__(self, vocab_size=1000, seq_len=64, num_pairs=8):
        ...
        assert vocab_size >= num_pairs, "Vocab size too small"
    ...
    def __iter__(self):
        ...
        while True:
            ...
            sequence.append(vals[query_idx]) # Target
            ...
            # Standard 任务不需要 loss_mask，走默认逻辑
...
class BlockRecallDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        while True:
            ...
            # Mask Logic: 标记 Prompt 位置 (QueryKey + Ans_1...Ans_{k-1})
...
def main():
    parser = argparse.ArgumentParser(description="Exp 1 Layer-wise: Synthetic Induction (Standard & Block)")
    ...
    parser.add_argument("--rope_scaling_threshold", type=int, default=-1,
                        help="Layers <= threshold use Standard RoPE. Layers > threshold use Scaled RoPE.")
    ...
    if not args.use_scaled_rope:
        mode_desc = "Baseline (Standard RoPE)"
    elif args.rope_scaling_threshold == -1:
        mode_desc = f"Uniform Scaled (Sigma={args.sigma})"
    else:
        mode_desc = f"Bio-Gradient (Thr={args.rope_scaling_threshold}, Deep Sigma={args.sigma})"
    
    print(f"Starting Run: {args.run_id} | Task: {args.task} | Mode: {mode_desc}")
    ...
    for step in range(args.steps):
        ...
        if (step + 1) % log_interval == 0:
            print(f"[{args.run_id}] Step {step+1}: Avg Loss {running_loss/log_interval:.4f} | Avg Acc {running_acc/log_interval:.4f}")
            running_loss = 0.0
            running_acc = 0.0
    ...
    with open(save_path, 'w') as f:
        json.dump({"config": vars(args), "history": history}, f, indent=4)
    
    # 保存权重 (用于 Length Extrapolation)
    torch.save(model.state_dict(), os.path.join(args.output_dir, f"{args.run_id}.pt"))
