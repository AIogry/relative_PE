...
def main():
    ...
    # === 可学习 Sigma 参数 ===
    parser.add_argument("--learnable_sigma", action="store_true", help="Enable learnable sigma for ScaledRoPE")
    ...
    parser.add_argument("--save_interval", type=int, default=1000, help="Save model every N steps")
    ...
    while step < total_steps:
        ...
        if step % LOG_INTERVAL == 0:
            ...
            if args.learnable_sigma and args.use_scaled_rope:
                sigma_dict = {}
                sigma_values = []
                # 遍历参数，提取层号和对应的 sigma 数组
                for name, param in model.named_parameters():
                    if 'sigma' in name:
                        # name 长这样: transformer.blocks.8.rotary_emb.sigma_param
                        # 我们可以通过字符串分割把层号提取出来
                        try:
                            layer_idx = name.split('.blocks.')[1].split('.')[0]
                            layer_name = f"Layer_{layer_idx}"
                        except IndexError:
                            layer_name = name # 提取失败时的 fallback
                            
                        vals = param.detach().cpu().numpy().tolist()
                        sigma_dict[layer_name] = [round(v, 4) for v in vals] # 保留4位小数方便查看
                        sigma_values.extend(vals)
                
                if sigma_values:
                    avg_sigma = sum(sigma_values) / len(sigma_values)
                    wandb.log({"train/avg_sigma": avg_sigma})
                    
                    # 每100步输出详细的按层归类的 sigma 值
                    if step % 100 == 0:
                        print(f"Step {step} | Sigma Detail:")
                        for k, v in sigma_dict.items():
                            print(f"  {k}: {v}")
                    else:
                        print(f"Step {step} | Avg Sigma: {avg_sigma:.4f}")
        ...
        # 每隔save_interval步保存模型
        if step % args.save_interval == 0 and step > 0:
            print(f">>> Saving model checkpoint at step {step}...")
            checkpoint_path = os.path.join(args.output_dir, f"model_step_{step}.pt")
            torch.save(model.state_dict(), checkpoint_path)
            wandb.save(checkpoint_path, base_path=args.output_dir)
    ...
    if args.learnable_sigma and args.use_scaled_rope:
        ...
        if sigma_values:
            print(f"Final Sigma Values: {sigma_values}")
            print(f"Average Sigma: {sum(sigma_values)/len(sigma_values):.4f}")
