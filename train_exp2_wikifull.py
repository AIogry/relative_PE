...
def main():
    ...
    parser.add_argument("--lr", type=float, default=6e-4) # 对于300M，建议外部bash传3e-4
    ...
    # 数据预处理与 DataLoader (保持你原有的完美逻辑不变)
    ...
    # === [修改点 2] 动态解耦网络层数、头数与维度 ===
    init_std_val = 0.02 # 默认初始化方差
    if args.model_size == "20M":
        ...
        cur_d, cur_h, cur_l, cur_mlp = 1024, 16, 24, 4
        init_std_val = 0.01 # 深层网络降低初始化方差，防止崩溃
        print(">>> Using OLMo-300M Configuration (24 Layers)")
    else:
        ...
        if step % args.eval_interval == 0:
            ...
            with torch.no_grad():
                for _ in range(args.eval_steps):
                    ...
                    with autocast(device_type='cuda', dtype=torch.bfloat16):
                        outputs = model(input_ids=vx.to(device))
                        loss = nn.functional.cross_entropy(outputs.logits.view(-1, vocab_size), vy.to(device).view(-1))
                    val_loss += loss.item()
                    val_count += 1
            if val_count > 0:
                print(f">>> VAL PPL: {math.exp(val_loss/val_count):.2f}")
            model.train()
