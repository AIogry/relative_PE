"""
Modified train.py with Gradient Accumulation
2026-1-14: Added Gradient Accumulation logic to ensure true Global Batch Size training.
"""
import torch
...
def run_experiment(cfg: TrainConfig, max_steps: int, log_interval: int, eval_interval : int, save_interval: int, device: torch.device, results_filename: str, use_streaming: bool):
    ...
    # --- [关键修改 1] 计算梯度累积步数 ---
    accum_steps = cfg.global_train_batch_size // cfg.device_train_microbatch_size
    if cfg.global_train_batch_size % cfg.device_train_microbatch_size != 0:
        raise ValueError(f"Global Batch Size ({cfg.global_train_batch_size}) must be divisible by Micro Batch Size ({cfg.device_train_microbatch_size})")
    
    print(f"Gradient Accumulation Enabled: {accum_steps} steps per update.")
    print(f"  - Micro Steps needed: {max_steps * accum_steps}")
    print(f"  - Global Steps target: {max_steps}")
    ...
    # --- evaluate and log ---
    def _evaluate_and_log(global_step):
        perplexity = evaluate_model(model, eval_loader, cfg, device, max_eval_steps=200)
        results[global_step] = perplexity
        print(f"Global Step [{global_step}], Perplexity: {perplexity:.4f}")
        # Save results immediately
        with open(results_filename, "w") as f:
            json.dump({cfg.run_name: results}, f, indent=4)
        print(f"Intermediate results saved to {results_filename}")

    # [关键修改 2] 重新计算总 Micro Steps
    total_micro_steps = max_steps * accum_steps
    
    optimizer.zero_grad()
    current_accum_loss = 0.0 # 用于记录累积后的平均 Loss

    # [关键修改 3] 循环逻辑变更：遍历 total_micro_steps
    for step, batch in enumerate(islice(train_loader, total_micro_steps)):
        model.train()
        ...
        # [关键修改 4] Loss Scaling & Accumulation
        scaled_loss = loss / accum_steps
        scaled_loss.backward()
        
        # 记录还原后的 Loss 用于显示
        current_accum_loss += scaled_loss.item()
        
        # [关键修改 5] 只有在 Accumulation 结束时才 Step
        if (step + 1) % accum_steps == 0:
            global_step = (step + 1) // accum_steps
            
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_grad_norm)
            optimizer.step()
            
            # 学习率调度 (按 Global Step)
            current_lr = scheduler.get_lr(cfg.optimizer.learning_rate, global_step, max_steps)
            for param_group in optimizer.param_groups:
                param_group['lr'] = current_lr
                
            optimizer.zero_grad()
            
            # --- Logging (按 Global Step) ---
            if global_step % log_interval == 0:
                print(f"Step [{global_step}/{max_steps}], Loss: {current_accum_loss:.4f}, LR: {current_lr:.2e}")
            
            # --- Eval & Save (按 Global Step) ---
            if global_step % eval_interval == 0:
                _evaluate_and_log(global_step)
    ...
    # Handle final step evaluation if not covered
    final_global_step = total_micro_steps // accum_steps
    if final_global_step % eval_interval != 0:
        _evaluate_and_log(final_global_step)
...
def main():
    ...
    parser.add_argument("--max_steps", type=int, default=10000, help="Total training steps (GLOBAL STEPS) for the experiment.")
    parser.add_argument("--save_interval", type=int, default=10000, help="Interval for saving checkpoints (in Global Steps).")
    ...
    parser.add_argument("--log_interval", type=int, default=100, help="Logging interval (in Global Steps).")
    parser.add_argument("--eval_interval", type=int, default=1000, help="Interval (in Global Steps) to evaluate and log perplexity.")
